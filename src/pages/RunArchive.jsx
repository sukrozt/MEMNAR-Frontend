export default function RunArchive({ logs = [] }) {
    ...
    <main className="flex-1 p-6 xl:p-8 overflow-y-auto">
      ...
      <div className="mt-8 space-y-6 max-w-7xl">
        ...
        <section className="bg-white rounded-3xl p-6 shadow-sm border border-[#ece7ef]">
          ...
          <div className="mt-6 bg-[var(--log-bg)] rounded-2xl p-4 font-mono text-sm h-[320px] overflow-auto">
            {hasLogs ? (
              logs.map((log, index) => (
                <div
                  key={index}
              ...
              <div className="text-[#8b7a93]">No archived logs yet</div>
            )}
          </div>
        </section>
